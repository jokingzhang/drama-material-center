...
4. Prefer the smallest compatible reference set. Compare visible content against the consumer's current look, action phase and camera side under [reference compatibility](../../../../PRODUCTION_SOP.md#reference-compatibility): footwear, held objects, prop count/state, furniture scale and doorway geography matter as well as identity. A correct hash or acceptance for another use does not settle these facts; repair an evidenced pixel conflict before generation. Reuse inspections when image, use and relevant facts are unchanged. Keep grids, floor plans, annotated boards, multi-panel action sheets, text-contaminated images, and other `INTERNAL` planning media out of generation inputs.
