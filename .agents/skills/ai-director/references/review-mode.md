...
- Compare the short scene design with the actual four-field body and edit consumers: important reactions remain visible at the chosen scale, depth/focus does not hide or prematurely reveal information, per-shot light agrees with world sources, and a sound bridge names a real source and covered picture (or an explicit pending task). Repair a missing carrier in that same pass; do not require a per-shot method ledger.
...
- Speech, breathing, turn-taking, sequential or overlapping actions and listener response plausibly fit the actual framing and time. Check what must happen in sequence; listing every step does not establish feasibility. Prioritize story-bearing changes and explicitly designed ellipses under [generation-unit design](../../../../PRODUCTION_SOP.md#generation-unit-design). Average character rate alone is insufficient; unperformed timing remains an estimate, not a demand to generate audio for text review.
...
- References contain compatible visible facts for the consumer's current look, action phase and camera side under [reference compatibility](../../../../PRODUCTION_SOP.md#reference-compatibility). Check changed uses even when the file/hash is unchanged; reuse same-use inspections. Missing/rejected/INTERNAL or conflicting inputs cannot be supplied as valid generation inputs. A label cannot hide conflicting pixels.
...
| Future model behavior is uncertain, with no evidenced input conflict | Name the trial unit, observable success and dependent work under [risk trials](../../../../PRODUCTION_SOP.md#risk-trials). Preserve text completion; do not claim effect validation or expand the dependent production on the strength of text checks. Honor the user's explicit trial/batch choice. |
...
Check the consequential effects selected in the scene design against the take and assembled sound. A focal or framing drift that preserves the intended information may be a note; an unrealized required reveal or explicitly requested camera effect must be reported accurately. Post-production-dependent effects remain pending until assembled and observed. Do not turn all directing preferences into new hard-reject conditions.
...
For a required effect, inspect its visible/audible success condition as well as general continuity: for example, stable subject scale with a changing background scale for a dolly zoom, full coverage at an occlusion cut, or voice and visible speaker ownership. If a hybrid route is used, check the composite and identify its production route. Do not infer these results from prompt terminology, structural PASS or task success.
...
Follow the selected author for authorized repairs; review-only stays read-only. Fix the earliest faulty layer using the [affected join window](../../../../PRODUCTION_SOP.md#repair-window) before and after repair. Unproduced neighbors retain an explicit untested join; inspected QA frames are not automatically approved model inputs. Stop searching for optional improvements when no major problem remains. If a fault persists without a feasible authorized correction, report it and continue unaffected work, rather than repeat the same attempt.
